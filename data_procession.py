...
def original_traj_consistency_check_plot(trj_in):
    current_seg_id = trj_in['segment_id'].iloc[0]
    follower_id_in = trj_in['local_veh_id'].iloc[0]
    correctness_dir = ['_x', '_y']
    for i in range(2):
        plt.subplot(3, 1, 1)
        plt.plot(trj_in['local_time'] * 0.04, trj_in['position' + correctness_dir[i]], '--k', linewidth=0.25, label='Original')
        ...
        trj_title = 'Scenario ' + str(int(current_seg_id)) + ' Vehicle' + str(
            int(follower_id_in)) + ' Direction' + correctness_dir[i] + ' Original Traj Consistency Check'
        plt.title(trj_title)
        plt.subplot(3, 1, 2)
        plt.plot(trj_in['local_time'] * 0.04, trj_in['origin_speed' + correctness_dir[i]], '--k', linewidth=0.5, label='Original')
        plt.plot(trj_in['local_time'] * 0.04, trj_in['speed' + correctness_dir[i]], '-m', linewidth=0.5, label='Position Based')
        plt.ylabel('Speed (m/s)')
        plt.legend(prop={'size': 6})
        if not i:
            plt.ylim([0, 50])
            ...
            plt.ylim([-10, 10])
        plt.subplot(3, 1, 3)
        plt.plot(trj_in['local_time'] * 0.04, trj_in['origin_accer' + correctness_dir[i]], '--k', linewidth=0.5, label='Original')
        plt.plot(trj_in['local_time'] * 0.04, trj_in['accer' + correctness_dir[i]], '-m', linewidth=0.5, label='Position Based')
        plt.legend(prop={'size': 6})
        plt.xlabel('Time (s)')
        plt.ylabel('Acceleration (m/s2)')
        if not i:
            ...
            plt.ylim([-5, 5])
        if not os.path.exists('figure_save/trajectory_process/original_traj_consistency_check_plot'):
            os.makedirs('figure_save/trajectory_process/original_traj_consistency_check_plot')
        trj_save_title = 'figure_save/trajectory_process/original_traj_consistency_check_plot/' + trj_title + '.png'
        plt.savefig(trj_save_title, dpi=600)
        plt.close('all')
...
def before_and_after_remove_outlier_plot(trj_in):
    ...
    for i in range(2):
        plt.subplot(3, 1, 1)
        plt.plot(trj_in['local_time'] * 0.04, trj_in['position' + correctness_dir[i]], '--k', linewidth=0.25, label='Original')
        plt.plot(trj_in['local_time'] * 0.04, trj_in['remove_outlier_pos' + correctness_dir[i]], '-m', linewidth=0.25, label='Outliers Removed')
        plt.ylabel('Position (m)')
        plt.legend(prop={'size': 6})
        trj_title = 'Scenario ' + str(int(current_seg_id)) + ' Vehicle' + str(
        ...
        plt.title(trj_title)
        plt.subplot(3, 1, 2)
        plt.plot(trj_in['local_time'] * 0.04, trj_in['speed' + correctness_dir[i]], '--k', linewidth=0.5, label='Original')
        plt.plot(trj_in['local_time'] * 0.04, trj_in['remove_outlier_speed' + correctness_dir[i]], '-m', linewidth=0.5, label='Outliers Removed')
        plt.ylabel('Speed (m/s)')
        plt.legend(prop={'size': 6})
        if not i:
            plt.ylim([0, 50])
            ...
            plt.ylim([-10, 10])
        plt.subplot(3, 1, 3)
        plt.plot(trj_in['local_time'] * 0.04, trj_in['accer' + correctness_dir[i]], '--k', linewidth=0.5, label='Original')
        plt.plot(trj_in['local_time'] * 0.04, trj_in['remove_outlier_accer' + correctness_dir[i]], '-m', linewidth=0.5, label='Outliers Removed')
        plt.legend(prop={'size': 6})
        plt.xlabel('Time (s)')
        plt.ylabel('Acceleration (m/s2)')
        if not i:
...
def before_and_after_filtering_plot(trj_in):
    ...
    for i in range(2):
        ...
        plt.plot(trj_in['local_time'] * 0.04, trj_in['position' + correctness_dir[i]], '--k', linewidth=0.25, label='Original')
        plt.plot(trj_in['local_time'] * 0.04, trj_in['remove_outlier_pos' + correctness_dir[i]], '-m', linewidth=0.25, label='Outliers Removed')
        plt.plot(trj_in['local_time'] * 0.04, trj_in['filter_pos' + correctness_dir[i]], '-*g', linewidth=0.25, label='Outliers Removed + Filtering', markersize=0.5)
        plt.ylabel('Position (m)')
        plt.legend(prop={'size': 6})
        trj_title = 'Scenario ' + str(int(current_seg_id)) + ' Vehicle' + str(
        ...
        plt.title(trj_title)
        plt.subplot(3, 1, 2)
        plt.plot(trj_in['local_time'] * 0.04, trj_in['speed' + correctness_dir[i]], '--k', linewidth=0.25, label='Original')
        plt.plot(trj_in['local_time'] * 0.04, trj_in['remove_outlier_speed' + correctness_dir[i]], '-m', linewidth=0.25, label='Outliers Removed')
        plt.plot(trj_in['local_time'] * 0.04, trj_in['filter_speed' + correctness_dir[i]], '-*g', linewidth=0.25, label='Outliers Removed + Filtering', markersize=0.5)
        plt.ylabel('Speed (m/s)')
        plt.legend(prop={'size': 6})
        if not i:
            plt.ylim([0, 50])
            ...
            plt.ylim([-10, 10])
        plt.subplot(3, 1, 3)
        plt.plot(trj_in['local_time'] * 0.04, trj_in['accer' + correctness_dir[i]], '--k', linewidth=0.25, label='Original')
        plt.plot(trj_in['local_time'] * 0.04, trj_in['remove_outlier_accer' + correctness_dir[i]], '-m', linewidth=0.25, label='Outliers Removed')
        plt.plot(trj_in['local_time'] * 0.04, trj_in['filter_accer' + correctness_dir[i]], '-*g', linewidth=0.25, label='Outliers Removed + Filtering', markersize=0.5)
        plt.legend(prop={'size': 6})
        plt.xlabel('Time (s)')
        plt.ylabel('Acceleration (m/s2)')
        if not i:
...
def optimization_based_outlier_removing(series_in, first_pos_in, last_pos_in, min_acc_in, max_acc_in, flag):
    '''基于优化模型的离群点剔除——在加速度曲线上识别，速度曲线上优化'''
    # 确定修正方向
    if flag == 0:
...
def trajectory_correctness(coord_series_in, segment_id_in, veh_id_in, start_time_in, all_outlier_record):
    ...
    minimum_accer = -8
    maximum_accer = 5
    total_related_points = 50  # 滑动窗口长度，离群点前后各10帧
...
def pair_cf_coord_cal(leader_id, leader_trj_in, follower_id, follower_trj_in, av_label, all_outlier_record):
    ...
    *************传入轨迹df的关键属性说明*****************
    ...
    :df_attri origin_speed_x                : 车辆的x向速度，对应highD中的xVelocity
    :df_attri origin_speed_y                : 车辆的y向速度，对应highD中的yVelocity
    ...
    *************传出轨迹df的关键属性说明*****************
    ...
    # 初始化输出格式
    out_leader_trj = pd.DataFrame(leader_trj_in[['segment_id', 'veh_id', 'length', 'local_time_stamp', 'origin_speed_x', 'origin_speed_y', 'origin_accer_x', 'origin_accer_y']].to_numpy(),
                                  columns=['segment_id', 'local_veh_id', 'length', 'local_time', 'origin_speed_x', 'origin_speed_y', 'origin_accer_x', 'origin_accer_y'])
    out_leader_trj.loc[:, 'follower_id'] = follower_id
    out_leader_trj.loc[:, 'leader_id'] = leader_id
    out_follower_trj = pd.DataFrame(follower_trj_in[['segment_id', 'veh_id', 'length', 'local_time_stamp', 'origin_speed_x', 'origin_speed_y', 'origin_accer_x', 'origin_accer_y']].to_numpy(),
                                    columns=['segment_id', 'local_veh_id', 'length', 'local_time', 'origin_speed_x', 'origin_speed_y', 'origin_accer_x', 'origin_accer_y'])
    ...
    leader_cumu_dis = cumulated_dis_cal(
        leader_trj_in.loc[:, ['global_center_x', 'global_center_y', 'origin_speed_x', 'origin_speed_y']], temp_current_segment_id, leader_id, temp_start_time, all_outlier_record)
    follower_cumu_dis = cumulated_dis_cal(
        follower_trj_in.loc[:, ['global_center_x', 'global_center_y', 'origin_speed_x', 'origin_speed_y']], temp_current_segment_id, follower_id, temp_start_time, all_outlier_record)
    ...
    original_traj_consistency_check_plot(out_follower_trj)
    original_traj_consistency_check_plot(out_leader_trj)
...
def directionUnify(df_ego, df_cutin):
    # 进行上下行方向的统一
    if df_ego.loc[0, 'global_center_x'] > df_ego.loc[len(df_ego) - 1, 'global_center_x']:
        ...
        df_ego.loc[:, 'origin_speed_x'] = df_ego.loc[:, 'origin_speed_x'] * -1
        df_ego.loc[:, 'origin_speed_y'] = df_ego.loc[:, 'origin_speed_y'] * -1
        df_cutin.loc[:, 'global_center_x'] = df_cutin.loc[:, 'global_center_x'] * -1
        df_cutin.loc[:, 'global_center_y'] = df_cutin.loc[:, 'global_center_y'] * -1
        df_cutin.loc[:, 'origin_speed_x'] = df_cutin.loc[:, 'origin_speed_x'] * -1
        df_cutin.loc[:, 'origin_speed_y'] = df_cutin.loc[:, 'origin_speed_y'] * -1
    return
...
if __name__ == '__main__':
    ...
    for i in range(len(scenario_path)):
        ...
        df_scene.rename(columns={
            'frame': 'local_time_stamp', 'id': 'veh_id', 'width': 'length', 'x': 'global_center_x', 'y': 'global_center_y', 'xVelocity': 'origin_speed_x',
            'yVelocity': 'origin_speed_y', 'xAcceleration': 'origin_accer_x', 'yAcceleration': 'origin_accer_y', 'precedingId': 'precedingId'}, inplace=True)
