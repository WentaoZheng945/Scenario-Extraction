...
def nds2xosc_V2(nds_path, xodr_path, output_path):
    '''
    该方法用于自动化实现NDS场景数据XML文件到OpenSCENARIO格式的转换
    转换思路：将每辆车的轨迹按照轨迹逐帧记录storyBoard中的Event中（NDS轨迹采样率为10HZ）
    【V2：根据EKT需求，对场景stopTrigger进行限制，场景长度不足30s的固定为30s】
    ...
    header.attrib = {'revMajor': '1', 'revMinor': '0', 'date': '2021-12-04T16:15:00', 'description': 'scenario_NDS', 'author': 'EKT'}
